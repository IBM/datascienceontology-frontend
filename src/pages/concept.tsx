...
const FunctionConceptDefList = (props: {
  concept: Concept.FunctionConcept;
}) => {
  const concept = props.concept;
  return [
    <dt key="dom-dt">Inputs</dt>,
    <dd key="dom-dd">
      <PortsDisplay ports={concept.inputs} />
    </dd>,
    <dt key="codom-dt">Outputs</dt>,
    <dd key="codom-dd">
      <PortsDisplay ports={concept.outputs} />
    </dd>
  ];
};
...
const PortsDisplay = (props: { ports: Concept.Port[] }) => {
  ...
  return (
    <ol className="has-padding-left-15">
      {ports.map((port, i) => (
        <li key={i}>
